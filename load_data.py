...
# 1. Parse .env file manually or from environment variables
env_vars = {}
...
supabase_url = os.environ.get("SUPABASE_URL") or env_vars.get("SUPABASE_URL", "").rstrip("/")
supabase_key = os.environ.get("SUPABASE_KEY") or env_vars.get("SUPABASE_KEY", "")

if not supabase_url or not supabase_key:
    print("\n[ERROR] SUPABASE_URL or SUPABASE_KEY missing in environment / .env file!")
    exit(1)

print(f"Connecting to Supabase REST API at: {supabase_url}")

# Configure persistent HTTP session with PostgREST merge-duplicates header for idempotent upserts
...
def upload_in_batches(table_name, records, batch_size=2000):
    """
    Uploads a list of dictionary records to a Supabase PostgREST table in batches.
    Uses merge-duplicates resolution to safely upsert existing records.
    """
...
def main():
    print("Starting SmartFactory Data Import to Supabase...\n")

    # Step 1: Machines Metadata
    print("[1/4] Processing Machines metadata...")
    try:
        ...
        smart_ids = smart_data["Machine ID"].dropna().unique()
        smart_machines = pd.DataFrame({
            "machine_id": [str(s_id) for s_id in smart_ids],
            "model": "Production Line",
            "age_years": None,
            "status": "Active"
        })

        all_machines = pd.concat([pdm_machines, smart_machines]).drop_duplicates(subset=["machine_id"]).reset_index(drop=True)
        ...
        print(f"[ERROR] Preparing machines failed: {e}")
        exit(1)
    ...
    try:
        dfs = []
        if os.path.exists("data/raw/PdM_maint.csv"):
            ...
            dfs.append(pdm_maint)

        if os.path.exists("data/raw/PdM_failures.csv"):
            ...
            dfs.append(pdm_failures)

        all_maint = pd.concat(dfs, ignore_index=True) if dfs else pd.DataFrame()
        ...
        print(f"[ERROR] Preparing maintenance logs failed: {e}")
        exit(1)
    ...
    try:
        ...
        defect_df["defective_units"] = (defect_df["production_output"] * defect_df["defect_rate"] / 100.0).round().astype(int)
        ...
        print(f"[ERROR] Preparing defect logs failed: {e}")
        exit(1)

    # Step 4: Telemetry / Uptime Logs
    ...
    try:
        ...
        print(f"[ERROR] Preparing telemetry logs failed: {e}")
        exit(1)
